...
import { csvField, escapeHtml } from "../utils/security.js";
...
export const exportAnalyticsCSV = async (req, res, next) => {
  try {
    ...
    const staffRows = staffPerf
      .map((e) => `${csvField(nameMap[e._id.toString()] || "Unknown")},${e.ticketsServed}`)
      .join("\n");
...
<body>
  <h1>${escapeHtml(getBrandSync().name)} Analytics Report</h1>
  <p class="subtitle">Branch: ${escapeHtml(branchId)} | Date: ${escapeHtml(report.date)}</p>
  <div class="stat-grid">
  ...
  <table>
    ...
    ${Object.entries(report.statusBreakdown).map(([k, v]) => `<tr><td>${escapeHtml(k)}</td><td>${v}</td></tr>`).join("")}
  </table>
  <h2>Staff Performance</h2>
  <table>
    <tr><th>Name</th><th>Tickets Served</th></tr>
    ${report.staffPerformance.map((s) => `<tr><td>${escapeHtml(s.name)}</td><td>${s.ticketsServed}</td></tr>`).join("")}
  </table>
  <p style="color:#999; margin-top:40px; font-size:12px;">Generated by ${escapeHtml(getBrandSync().name)} Queue Management System</p>
</body></html>`;
